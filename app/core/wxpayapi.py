    ...
    def __new__(cls, *args, **kwargs):
        if not hasattr(cls, "_instance"):
            with cls._instance_lock:
                if not hasattr(cls, "_instance"):
                    impl = cls.configure() if hasattr(
                        cls, "configure") else cls
...
class UnifiedOrder_pub(Wxpay_client_pub):
    ...
    def getPrepayId(self):
        ...
        self.postXml()
        self.result = self.xmlToArray(self.response)
        print self.result['return_code'] + '____' + self.result['return_msg'].encode('gbk')
...
def test():
    ...
    c2 = HttpClient()
    assert id(c) == id(c2)
    print c.get("http://192.168.199.239:5000/api/v1.0/discovery/index/list")
